...
describe ActiveRecord::Migrator do
  ...
  it "test_valid_migration" do
    ...
    SwapOutput.stdout{ ActiveRecord::Migrator.down(migration_path, 0) }
    # expect(SchemaComments::SchemaComment.count).to eq 0

    SwapOutput.stdout{ ActiveRecord::Migrator.up(migration_path, 1) }
    SwapOutput.stdout{ ActiveRecord::Migrator.up(migration_path, 2) }
    ...
    SwapOutput.stdout{ ActiveRecord::Migrator.down(migration_path, 1) }
    expect(ActiveRecord::Migrator.current_version).to eq 1
    ...
    SwapOutput.stdout{ ActiveRecord::Migrator.up(migration_path, 4) }
    ...
    SwapOutput.stdout{ ActiveRecord::Migrator.down(migration_path, 3) }
    expect(ActiveRecord::Migrator.current_version).to eq 3
    # expect(SchemaComments::SchemaComment.count).to eq 6

    SwapOutput.stdout{ ActiveRecord::Migrator.up(migration_path, 5) }
    expect(ActiveRecord::Migrator.current_version).to eq 5
    SchemaComments.clear_cache
    expect(SchemaComments.column_comment('products', 'name')).to eq '商品名'

    SwapOutput.stdout{ ActiveRecord::Migrator.up(migration_path, 6) }
    ...
    SwapOutput.stdout{ ActiveRecord::Migrator.up(migration_path, 7) }
    ...
    SwapOutput.stdout{ ActiveRecord::Migrator.up(migration_path, 9) }
